...
from svg_preset_authoring.authoring import (
...
_TEXT_SHAPE_ATTR = 'data-pptx-text-shape'


def _shape_text_children(
    group: ET.Element,
) -> tuple[ET.Element, ET.Element] | None:
    """Return one explicit geometry/text pair that must compile to one p:sp."""
    marker = group.get(_TEXT_SHAPE_ATTR)
    if marker is None:
        return None
    if marker != 'true':
        raise SvgNativeConversionError(
            f'{_TEXT_SHAPE_ATTR} must equal "true"'
        )
    if group.tag.replace(f'{{{SVG_NS}}}', '') != 'g':
        raise SvgNativeConversionError(
            f'{_TEXT_SHAPE_ATTR} belongs on an SVG <g>'
        )
    if not (group.get('id') or '').strip():
        raise SvgNativeConversionError(
            f'{_TEXT_SHAPE_ATTR} group requires a stable id'
        )
    forbidden_group_attrs = (
        'transform', 'filter', 'opacity', 'style', 'fill', 'stroke',
        'data-pptx-layer', 'data-pptx-placeholder', 'data-pptx-carrier',
        'data-pptx-binding',
    )
    present = [name for name in forbidden_group_attrs if group.get(name) is not None]
    if present:
        raise SvgNativeConversionError(
            f'{_TEXT_SHAPE_ATTR} is Slide-local and frame-owning; move '
            'presentation/structure attributes to its children: '
            + ', '.join(present)
        )
    visual = [
    ...
    if len(visual) != 2:
        raise SvgNativeConversionError(
            f'{_TEXT_SHAPE_ATTR} requires exactly one geometry child followed '
            'by exactly one direct <text>'
        )
    geometry, text = visual
    if text.tag.replace(f'{{{SVG_NS}}}', '') != 'text':
        raise SvgNativeConversionError(
            f'{_TEXT_SHAPE_ATTR} requires the direct <text> to be the second '
            'visual child'
        )
    transformed_children = [
        child.tag.replace(f'{{{SVG_NS}}}', '')
        for child in (geometry, text)
        if child.get('transform') is not None
    ]
    if transformed_children:
        raise SvgNativeConversionError(
            f'{_TEXT_SHAPE_ATTR} geometry/text children must use explicit '
            'coordinates, not transform: ' + ', '.join(transformed_children)
        )
    if text.get('data-pptx-frame') is not None:
        raise SvgNativeConversionError(
            f'{_TEXT_SHAPE_ATTR} owns the text frame; remove data-pptx-frame '
            'from the child <text>'
        )
    if text.get('filter') is not None:
        raise SvgNativeConversionError(
            f'{_TEXT_SHAPE_ATTR} does not support a separate text filter'
        )
    geometry_tag = geometry.tag.replace(f'{{{SVG_NS}}}', '')
    ordinary_geometry = geometry_tag in {'rect', 'circle', 'ellipse'}
    authored_preset = (
        geometry_tag == 'g'
        and geometry.get('data-pptx-authoring') == 'preset'
        and geometry.get('data-pptx-object') == 'shape'
        and geometry.get('data-pptx-prst') is not None
        and geometry.get('data-pptx-frame') is not None
    )
    if not ordinary_geometry and not authored_preset:
        raise SvgNativeConversionError(
            f'{_TEXT_SHAPE_ATTR} geometry must be one rect, circle, ellipse, '
            'or validated authored preset shape'
        )
    return geometry, text


def _shape_text_frame(geometry: ET.Element) -> str:
    """Return the geometry frame in the marker group's local SVG space."""
    tag = geometry.tag.replace(f'{{{SVG_NS}}}', '')
    if tag == 'g':
        frame = geometry.get('data-pptx-frame')
        if frame is None:
            raise SvgNativeConversionError(
                'Authored preset text shape requires data-pptx-frame'
            )
        return frame

    def length(name: str, default: float = 0.0) -> float:
        raw = geometry.get(name)
        if raw is not None and re.fullmatch(
            r'[+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:px)?', raw.strip()
        ) is None:
            raise SvgNativeConversionError(
                f'{_TEXT_SHAPE_ATTR} requires numeric {name}, got {raw!r}'
            )
        return parse_svg_length(raw, default)

    if tag == 'rect':
        x, y = length('x'), length('y')
        width, height = length('width'), length('height')
    elif tag == 'circle':
        cx, cy, radius = length('cx'), length('cy'), length('r')
        x, y, width, height = cx - radius, cy - radius, radius * 2, radius * 2
    else:
        cx, cy = length('cx'), length('cy')
        rx_raw, ry_raw = geometry.get('rx'), geometry.get('ry')
        rx = length('rx') if rx_raw is not None else length('ry')
        ry = length('ry') if ry_raw is not None else length('rx')
        x, y, width, height = cx - rx, cy - ry, rx * 2, ry * 2
    if width <= 0 or height <= 0:
        raise SvgNativeConversionError(
            f'{_TEXT_SHAPE_ATTR} geometry frame must be positive'
        )
    return ' '.join(format(value, '.12g') for value in (x, y, width, height))


def _text_body_xml(shape: ShapeResult) -> str:
    """Extract the existing converter-owned txBody without changing its rules."""
    start = shape.xml.find('<p:txBody>')
    end = shape.xml.find('</p:txBody>')
    if start < 0 or end < 0:
        raise SvgNativeConversionError(
            f'{_TEXT_SHAPE_ATTR} text child did not produce a native txBody'
        )
    return shape.xml[start:end + len('</p:txBody>')]
...
def convert_g(elem: ET.Element, ctx: ConvertContext) -> ShapeResult | None:
    ...
    shape_text_pair = _shape_text_children(elem)
    if shape_text_pair is not None:
        geometry, text = shape_text_pair
        geometry_result = convert_element(geometry, child_ctx)
        if geometry_result is None or not geometry_result.xml.lstrip().startswith('<p:sp>'):
            raise SvgNativeConversionError(
                f'{_TEXT_SHAPE_ATTR} geometry did not produce one native p:sp'
            )
        frame = _shape_text_frame(geometry)
        text.set('data-pptx-frame', frame)
        try:
            text_result = convert_element(text, child_ctx)
        finally:
            text.attrib.pop('data-pptx-frame', None)
        ctx.sync_from_child(child_ctx)
        if text_result is None:
            raise SvgNativeConversionError(
                f'{_TEXT_SHAPE_ATTR} requires non-empty visible text'
            )
        merged = _append_shape_text(
            geometry_result,
            _text_body_xml(text_result),
        )
        if ctx.trace_events is not None:
            shape_match = re.search(r'<p:cNvPr id="(\d+)"', merged.xml)
            ctx.trace_events.append({
                'tag': 'g',
                'id': elem_id,
                _TEXT_SHAPE_ATTR: 'true',
                'decision': 'merge-shape-text',
                'shape_id': int(shape_match.group(1)) if shape_match else None,
            })
...
def convert_svg_to_slide_shapes(
    ...
    # Normalize positional <tspan> content before conversion. In merge mode,
    # consistent absolute-y or relative-dy wrapped lines stay under one
    # annotated <text> and become one native PowerPoint text frame. Only
    # non-mergeable positional content falls back to independent <text>
    # elements; the final SVG gate rejects that fallback in release exports.
    ...
    if flattened:
        trace_steps.append({
            'action': 'normalize-positional-tspans',
            'merge_paragraphs': merge_paragraphs,
        })
        if verbose:
            print('  Normalized positional <tspan> text-frame topology')
