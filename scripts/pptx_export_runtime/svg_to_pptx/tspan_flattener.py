...
def flatten_positional_tspans(
    ...
    as a single <text> for downstream multi-<a:p> conversion. Set
    ``split_unmergeable`` to False for a normalization-only pass that must
    leave unresolved blocks available to a later topology gate.
    ...
    return flatten_text_with_tspans(
        ...
        split_unmergeable=split_unmergeable,
    )
