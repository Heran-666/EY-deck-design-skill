...
class SVGQualityChecker:
        ...
        *,
        ...
        try:
            ...
            if root is not None:
                ...
                self._check_text_elements(content, root, result, svg_path)
    ...
    def _check_text_elements(
        ...
        self._check_module_bounds_contract(root, result, svg_path)
        self._check_text_output_geometry(root, result)
        self._check_root_module_text_bounds(root, result)
        self._check_textbox_topology_preservation(root, result, svg_path)
        self._check_fragmented_paragraph_text(root, result, svg_path)
        self._check_unmergeable_leading_text(root, result)

    def _text_topology_issue_bucket(
        self,
        result: Dict,
        svg_path: Path,
    ) -> List[str]:
        """Return the stage-appropriate channel for text-frame topology."""
        if self.stage == 'final' and not self.template_mode:
            return result['errors']
        return result['warnings']

    def _check_textbox_topology_preservation(
        self,
        root: ET.Element,
        result: Dict,
        svg_path: Path,
    ) -> None:
        """Require one authored ``<text>`` to stay one native text frame.

        Native merge-mode export first normalizes positional ``<tspan>``
        lines.  When the paragraph classifier rejects an authored block, the
        fallback creates sibling ``<text>`` elements and PowerPoint therefore
        receives several editable text boxes.  Reuse that exact transform on
        isolated copies so the final SVG gate catches the topology change
        ...
        if _flatten_positional_tspans is None:
            bucket.append(
                'Unable to import positional-tspan normalization; text-frame '
                'topology cannot be verified before native PPTX export'
            )
            return
        ...
        for text_el in root.iter(f'{{{SVG_NS}}}text'):
            if (
                self._is_hidden_element(text_el, parent_by_id)
            ...
            wrapper.append(copy.deepcopy(text_el))
            isolated_root = ET.Element(f'{{{SVG_NS}}}svg')
            isolated_root.append(wrapper)
            try:
                _flatten_positional_tspans(
                    ET.ElementTree(isolated_root),
                    merge_paragraphs=True,
                )
            except (TypeError, ValueError, ET.ParseError) as exc:
                violations.append(f'{_element_label(text_el)} ({exc})')
                continue

            output_count = sum(
                1
                for element in wrapper.iter(f'{{{SVG_NS}}}text')
            )
            if output_count > 1:
                violations.append(
                    f'{_element_label(text_el)} -> {output_count} text boxes'
                )
        ...
        suffix = (
            ''
            if len(violations) <= 3
            else f'; +{len(violations) - 3} more'
        )
        bucket.append(
            f'Text-frame topology violation: {len(violations)} authored '
            f'<text> element(s) would split during merge-mode native export '
            f'({sample}{suffix}). One intended PowerPoint text box must remain '
            'one SVG <text>: use direct <tspan> line children with the parent '
            'x and either consistently increasing absolute y baselines '
            'starting at the parent y or consistent positive relative dy '
            'steps. Author semantically independent labels as separate text '
            'elements in separate logical groups.'
        )
    ...
    def _check_module_bounds_contract(
        ...
        if missing:
            ...
            bucket = (
                result['errors']
                if require_bounds and self._confirmed_export_manifest(svg_path) is None
                else result['warnings']
            )
    ...
    def _check_fragmented_paragraph_text(
        ...
        self._text_topology_issue_bucket(result, svg_path).append(
    ...
    def _confirmed_export_manifest(self, svg_path: Path) -> dict | None:
        """Return the manifest only when it hash-binds this normalized page."""
        manifest_path = svg_path.parent.parent / 'confirmed-svg-export.json'
        ...
        try:
            ...
            return None
        if manifest.get('schema') != 'ey-deck.confirmed-svg-export-runtime.v1':
            return None
        page = next(
            (
                item for item in manifest.get('page_order', [])
                if isinstance(item, dict) and item.get('page') == svg_path.stem
            ),
            None,
        )
        ...
        try:
            ...
            return None
        if page.get('normalized_sha256') != digest:
            return None
        return manifest

    def _confirmed_export_observed_colors(self, svg_path: Path) -> set[str]:
        """Return hash-bound paints inherited from confirmed EY SVGs.

        Confirmed-export ``spec_lock.md`` is synthesized only to describe the
        isolated technical workspace; it is not a new semantic color registry.
        Every valid paint already present in a hash-bound confirmed page is
        therefore inherited input and must not become Stage 2 color drift.
        """
        manifest = self._confirmed_export_manifest(svg_path)
        if manifest is None:
            return set()
        observed = manifest.get('observed_visual_values', {})
        raw_colors = observed.get('colors', {}) if isinstance(observed, dict) else {}
        if not isinstance(raw_colors, dict):
            return set()
        colors: set[str] = set()
        for raw_value in raw_colors:
            color = _normalize_hex_rgb(str(raw_value))
            if color:
                colors.add(color)
        return colors
    ...
    def _check_spec_lock_alignment(
        ...
        *,
        ...
        confirmed_colors = self._confirmed_export_observed_colors(svg_path)
        allowed_colors.update(prototype_colors)
        allowed_colors.update(confirmed_colors)
        ...
        for attr in _PAINT_PROPERTIES or ():
            for raw_value in self._svg_property_values(content, attr):
                ...
                if val not in allowed_colors:
                    ...
                    val in prototype_colors or val in confirmed_colors
                ) and val not in locked_colors:
        ...
        if inherited_parts:
            self._append_inherited_info(
                result,
                'spec_lock_alignment',
                f"{', '.join(inherited_parts)} come unchanged from hash-bound "
                "inherited input (confirmed SVG or mirror prototype) and are "
                "accepted without expanding spec_lock.md",
            )
    ...
    def _categorize_issue(self, error_msg: str) -> str:
        ...
        elif 'text-frame topology' in error_msg.lower() or 'paragraph-like line run' in error_msg.lower():
            return 'Text-frame topology'
