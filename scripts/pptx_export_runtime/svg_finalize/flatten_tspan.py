...
def _classify_paragraph_block(
    ...
    Conditions (all must hold):
      ...
      - Lines use either absolute y coordinates throughout or relative dy
        coordinates throughout. In absolute-y form, the first line matches
        the parent text baseline and later baselines increase. In relative-dy
        form, the first line has dy == 0 (or no dy) and later lines use
        positive dy.
      - Derived line gaps cluster around a single minimum "base line-height";
    ...
    # First pass: validate per-line structural rules shared by absolute-y and
    # relative-dy authoring forms. Both are normal SVG ways to place wrapped
    # lines and both must preserve one authored <text> as one PPTX text frame.
    starters = [group[0] for group in line_groups]
    ...
    uses_absolute_y = [get_attr(tspan, "y") is not None for tspan in starters]
    line_gaps: list[float] = [0.0]
    if any(uses_absolute_y):
        # Do not infer through a mixed absolute/relative sequence; such a
        # sequence can encode deliberate jumps. A consistent absolute-y block
        # is unambiguous: convert baseline deltas into paragraph line gaps.
        if not all(uses_absolute_y):
            return None
        base_y = parse_first_number(get_attr(text_el, "y"))
        baselines: list[float] = []
        for tspan in starters:
            baseline = parse_first_number(get_attr(tspan, "y"))
            t_dy = parse_first_number(get_attr(tspan, "dy"))
            if baseline is None or (t_dy is not None and abs(t_dy) > 1e-6):
                return None
            baselines.append(baseline)
        if base_y is None or abs(baselines[0] - base_y) > DY_TOLERANCE_PX:
            return None
        ...
        for idx, tspan in enumerate(starters):
            t_dy_raw = get_attr(tspan, "dy")
            t_dy = (
                parse_first_number(t_dy_raw)
                if t_dy_raw is not None
                else None
            )
            if idx == 0:
                if t_dy is not None and abs(t_dy) > 1e-6:
                    return None
            elif t_dy is None or t_dy <= 0:
                return None
            else:
                line_gaps.append(t_dy)

    # Second pass: pick the minimum positive gap as the base line-height and
    # express each line's gap as base + extra space-before.
    positive_gaps = [gap for gap in line_gaps[1:] if gap > 0]
    if not positive_gaps:
        return None
    base = min(positive_gaps)
    ...
    for idx, d in enumerate(line_gaps[1:], start=1):
