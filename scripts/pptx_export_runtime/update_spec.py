...
def parse_lock(lock_path: Path) -> dict[str, dict[str, str]]:
    """Parse the section/key subset consumed by the bundled export runtime.

    Keeping this parser local avoids importing PPT Master's broader project
    management surface just to read ``canvas.viewBox`` during conversion.
    """
    ...
    for raw in lock_path.read_text(encoding="utf-8").splitlines():
        ...
        if section is None:
            continue
        field = re.match(r"^\s*-\s+([^:]+?)\s*:\s*(.*?)\s*$", raw)
        if field:
            key = field.group(1).strip()
            if key in parsed[section]:
                raise ValueError(f"duplicate field {section}.{key}")
            parsed[section][key] = field.group(2).strip()
    return parsed
