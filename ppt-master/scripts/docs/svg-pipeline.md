...
writes anything. The destination `templates/` directory must be absent or
empty; any existing entry blocks direct materialization. Create Template uses
its isolated transition workspace when a new Layout or Deck must be composed
with the other structural kind. It stages the whole
