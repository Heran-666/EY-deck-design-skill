...
- **Hard rule — one paragraph, one text frame**: use one `<text>` per prose paragraph, never sibling `<text>` elements for its visual lines. Inline-formatting `<tspan>` runs carry no `x`, `y`, `dx`, or `dy`; word boundaries are literal text characters, never visual `dx` offsets. Author multiline carriers entirely as direct-child rows: every row repeats the parent `x`, the first uses `dy="0"`, and later rows use positive relative `dy`. Absolute `y` is accepted only when the converter can deterministically normalize a same-x, strictly increasing, transform-free stack without moving its first baseline; otherwise completion blocks. Default retains these breaks without PowerPoint wrapping; `--reflow-text` enables reflow. Start from the shared leading ranges in [`shared-standards-core.md`](./shared-standards-core.md) §4.2, then adjust for the typeface, reading distance, explicit user/template requirements, and locked style.
