...
**`page_layouts` (`mirror` / `layout` only)**: before drawing, resolve the current page's complete prototype SVG under §1.0's mapping and read/reuse rules. A missing section or row, or a basename that does not name a real file, stops with an upstream contract error; neither strict nor adaptive falls back to free design. Under `style` the section must be absent. Never invent an entry or assume structure because `templates/` exists. A `reference_set` fingerprint may diagnose an uncertain path but is not required.
